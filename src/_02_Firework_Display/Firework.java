        ...
        public void updateSparks() {
            // 8. Uncomment this code. See if you can understand what it is doing.
             ...
             for(int i = 0; i < sparks.length; i++) {
                 ...
                 Spark s = sparks[i];
                 s.x += s.xVelocity;  //sparks are equal to the xy velocity
                 s.y += s.yVelocity;
                 s.yVelocity += GRAVITY;  //y velocity is equal to the gravity
             
                 if(s.yVelocity >= 0 && !s.fire) {  //fire if you haven't fired yet and if the yvelocity is greater than 0
                     Random r = new Random();
